...
function NEDB(databaseName) {
  const options = {
    filename: path.join(STORE_PATH, `czr_${databaseName}.db`),
    // filename: path.join("./build/data", `czr_${databaseName}.db`),
    autoload: true,
    timestampData: true
  };
...
NEDB.prototype.compactDatafile = function() {
  this.nedb.persistence.compactDatafile();
};

module.exports = databaseName => {
  return new NEDB(databaseName);
};
