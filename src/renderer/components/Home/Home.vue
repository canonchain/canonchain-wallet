<template>
  <div class="page-czr-home">
    <div class="home-banner">
      ...
      <div>
        <el-button size="small" plain @click="dialogSwitch.import = true"
          >{{ $t("page_home.import_account") }}
        </el-button>
      </div>
    </div>
      ...
      <div class="account-wrap b-flex">
        <template v-for="account in database">
          ...
          >
            ...
            <i
              class="iconfont delete-acc"
              @click.stop="showRemoveDia(account.address)"
              >&#xe613;</i
            >
        ...
        <!--  ADD  -->
        <div
          class="accounrt-item add-account"
          @click="dialogSwitch.create = true"
        >
          <div class="account-cont">
    ...
    <el-dialog
      ...
      :title="
        createInfo.step === 0
          ? $t('page_home.create_dia.create_tit')
          : $t('page_home.create_dia.backup_tit')
      "
    ...
    >
      <template v-if="createInfo.step === 0">
        ...
        <el-input
          v-model="createInfo.tag"
          :placeholder="$t('page_home.create_dia.placeholder_tag')"
        >
          <template slot="prepend">
            <i class="el-icon-tickets"></i>
            {{ $t("page_home.create_dia.create_tag") }}
          </template>
        </el-input>
        <el-input
          v-model="createInfo.pwd"
          :placeholder="$t('page_home.create_dia.placeholder_pwd')"
          type="password"
        >
          <template slot="prepend">
            <i class="el-icon-edit"></i>
            {{ $t("page_home.create_dia.create_pwd") }}
          </template>
        </el-input>
        <el-input
          v-model="createInfo.repwd"
          :placeholder="$t('page_home.create_dia.placeholder_repwd')"
          type="password"
        >
          <template slot="prepend">
            <i class="el-icon-edit"></i>
            {{ $t("page_home.create_dia.create_repwd") }}
          </template>
        </el-input>
        <div slot="footer">
          <el-button @click="dialogSwitch.create = false">{{
            $t("cancel")
          }}</el-button>
          <el-button
            type="primary"
      ...
      <template v-if="createInfo.step === 1">
        <el-alert
          :title="$t('page_home.create_dia.create_success')"
          :description="$t('page_home.create_dia.create_success_des')"
          :closable="false"
          type="success"
          show-icon
        >
        ...
        <el-alert
          :title="$t('page_home.create_dia.download_tit')"
          :description="$t('page_home.create_dia.download_des')"
          :closable="false"
          type="warning"
          show-icon
        >
        ...
        <div slot="footer">
          <el-button
            type="danger"
            @click="downloadKeystore(createInfo.address)"
          >
        ...
        <template>
          ...
          >
            ...
            <el-button
              size="mini"
              @click="selectImport"
              :disabled="selectImporting"
              >选择文件</el-button
            >
          </div>
          <el-input
            v-model="importInfo.tag"
            :placeholder="$t('page_home.import_dia.placeholder_tag')"
          >
            <template slot="prepend">
              <i class="el-icon-document"></i>
              {{ $t("page_home.import_dia.create_tag") }}
            </template>
          </el-input>
        </template>
        <div slot="footer">
          <el-button @click="dialogSwitch.import = false">{{
            $t("cancel")
          }}</el-button>
    ...
    >
      <span>
        <el-alert
          v-if="removeInfo.alert"
          :title="removeInfo.alert.content"
          :closable="false"
          :type="removeInfo.alert.type"
          show-icon
        >
        </el-alert>
        ...
        <el-input
          v-model="removeInfo.pwd"
          :placeholder="$t('page_home.create_dia.placeholder_repwd')"
          type="password"
        >
          <template slot="prepend">
            <i class="el-icon-edit"></i>
            {{ $t("page_home.create_dia.create_repwd") }}</template
          >
        </el-input>
      </span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogSwitch.remove = false">{{
          $t("cancel")
        }}</el-button>
        <el-button type="danger" @click="removeAccountFn()">{{
          $t("page_home.remove_dia.remove_confrim")
        }}</el-button>
      </span>
    </el-dialog>
  </div>
...
let aryForBalans = [];
// const accountErrorTimer = null;
const updataBlockTimer = {
  start: null,
  error: null
};
const updataBlockData = {
  sourcesObj: null, // 源文件
...
app.on("quit", () => {
  ...
  const currentPid = Number(sessionStorage.getItem("CanonchainPid"));
  const result = process.kill(currentPid, "SIGINT");
...
export default {
  ...
  methods: {
      ...
      dialog.showOpenDialog(
        {
          title: "请选择keytore文件",
          properties: ["openFile"],
          filters: [{ name: "Account File", extensions: ["json"] }]
        },
        filePaths => {
          if (filePaths) {
            // hack importKeystore
    ...
    async initAccount(params) {
      ...
      //     .value();
      const account = await this.$nedb.account.findOne({
        address: params.address
      });
      // console.log("account",account)
      if (account) {
        this.$message.error(
          `${this.$t("page_home.import_dia.exist")}"${account.tag}"`
        );
        return;
      }
      ...
      //     .push(params)
      //     .write();
      await this.$nedb.account.insert(params);
    ...
    async pushKeystore(keystore) {
      ...
      //     .push(keystore)
      //     .write();
      await this.$nedb.accounts_keystore.insert(keystore);
    ...
    async createAccount() {
      if (self.database.length >= 50) {
        self.$message.error(
          self.$t("page_home.create_dia.account_quantity_error")
        );
        return;
      }
      if (!this.createInfo.tag) {
        this.createInfo.error = this.$t("page_home.create_dia.validate_tag");
        return;
      }
      ...
      const reg = /^[0-9a-zA-Z!@#$%^&*]{8,100}$/g;
      if (!reg.test(this.createInfo.pwd)) {
        this.createInfo.error = this.$t(
          "page_home.create_dia.validate_type_password"
        );
        return;
      }
      ...
      const accountResult = await self.$czr.accounts.create(
        self.createInfo.pwd
      );
      if (accountResult.account) {
        ...
        const params = {
          ...
          tag:
            self.createInfo.tag ||
            self.$t("page_home.acc") + (self.database.length + 1),
          balance: "0",
          send_list: []
        };
        // loading
        ...
        await this.$nedb.account.done();
        await this.$nedb.accounts_keystore.done();
        await this.$nedb.account.done();
    ...
    downloadKeystore(accVal) {
      ...
      const blob = new Blob([self.createInfo.keystore]);
    ...
    importKeystore(event) {
      ...
      if (fileLength !== 1) {
        self.$message.error(
          self.$t("page_home.import_dia.keystore_length_error")
        );
        return;
      }
      ...
      fs.readFile(targetFile.path, "utf8", async (err, data) => {
        ...
        if (!targetJson) {
          self.$message.error(
            self.$t("page_home.import_dia.keystore_format_error")
          );
          return;
        }
        ...
        const importObjec = await this.$nedb.account.findOne({
          address: targetJson.account
        });

        if (importObjec) {
          self.$message.error(
            `${self.$t("page_home.import_dia.keystore_is_has")} : ${
              targetJson.account
            }`
          );
          return;
    ...
    importAccount() {
      ...
      // const account = null;
      const importObj = JSON.parse(self.importInfo.keystore);
      ...
      self.$message.success(
        self.$t("page_home.import_dia.imported_account_success")
      );
      self.initDatabase();
    },
    // Import End
    ...
    async removeAccountFn() {
      ...
      //     .value();
      const isKeystoreAccount = await this.$nedb.accounts_keystore.findOne({
        account: self.removeInfo.address
      });
      if (isKeystoreAccount) {
        // 删除本地账户
        // const accountResult = ipcRenderer.sendSync('remove_account', isKeystoreAccount, self.removeInfo.pwd);
        const accountResult = await self.$czr.accounts.validate_account(
          isKeystoreAccount,
          self.removeInfo.pwd
        );
        ...
        if (accountResult) {
          ...
          self.$message.error(
            self.$t("page_home.remove_dia.validate_password")
          );
        }
      } else {
    ...
    async removeSuccess() {
      ...
      //     .write();
      await this.$nedb.account.remove({
        address: self.removeInfo.address
      });
      await this.$nedb.accounts_keystore.remove({
        account: self.removeInfo.address
      });
    ...
    async getAccountsBalances(accounts) {
      self.$czr.request
        ...
        .then(async res => {
          ...
          if (res.code !== 0) {
            return self.$walletLogs.info(`Accounts Balances Error ${res.msg}`);
          }
          // let localAcc;
          let balance;
          function updateBalance(balance, account) {
            return new Promise(async (resolve, reject) => {
              this.$nedb.account
                ...
                .then(accObj => {
                  if (accObj && accObj.balance !== balance) {
                    this.$nedb.account
                      .update({ address: account }, { $set: { balance } })
                      .then(ret => {
                        this.$walletLogs.info("updateBalance succeed", ret);
                        resolve(true);
                      })
                      .catch(err => {
                        reject(err);
              ...
              try {
                const accObj = await this.$nedb.account.findOne({
                  address: account
                });
                if (accObj && accObj.balance !== balance) {
                  await this.$nedb.account.update(
                    { address: account },
                    { $set: { balance } }
                  );
                }
          ...
          for (
            let localIndex = 0, len = res.balances.length;
            localIndex < len;
            localIndex += 1
          ) {
            ...
            updateBalance(balance, account)
              ...
              .catch(err => {
                this.$walletLogs.error("updateBalance error", err.stack);
              });
            // localAcc = await this.$nedb.account.findOne({
            //   address: accounts[localIndex]
            // });
          ...
          //     //     .read()
          //     //     .get("czr_accounts")
          //     //     .find({address: accounts[i]})
          //     //     .assign({balance: new BigNumber(balance).toString()})
          //     //     .write()
          //     localAcc = await this.$nedb.account.findOne({address: accounts[i]});
          //     console.log("localAcc", localAcc.balance.toString(),balance.toString())
          //     if(localAcc && (localAcc.balance.toString() !== balance.toString())){
          //         console.log("xiugai",accounts[i])
          //         let aloneUpdateRes = await this.$nedb.account.update({address: accounts[i]},{ $set: { balance: Number(balance) } });
          //     }
          // })
        })
        .catch(error => {
    ...
    async chooseUpdateBlocksData() {
      ...
      updataBlockData.sourcesObj = await self.$nedb.account_tx.find({
        is_stable: "0"
      });

      let curentAry = [];
      updataBlockData.targetAry = [];
      Object.keys(updataBlockData.sourcesObj).forEach(key => {
        curentAry = curentAry.concat(updataBlockData.sourcesObj[key]);
      });
      // for (const key in updataBlockData.sourcesObj) {
      //   curentAry = curentAry.concat(updataBlockData.sourcesObj[key]);
      // }
        ...
        .then(data => {
          ...
          obtainData.forEach(async ele => {
            if (ele.is_stable === "1") {
              ...
              // const aloneUpdateBlock = await this.$nedb.account_tx.update(
