<template>
  <div class="page-transfer" v-loading="loading">
      ...
      <el-form ref="form" label-width="100px" v-if="database.length">
        <el-form-item :label="$t('page_transfer.from_address')">
          <el-select
            ...
            :placeholder="$t('page_transfer.select')"
            style="width:100%;"
          >
            <el-option
        ...
        <el-form-item :label="$t('page_transfer.gas')">
          <el-input
            ...
            :value="gas"
            :min="0"
            :max="accountInfo.balance / (gasPrice * 1000000000)"
            class="width-180"
          ></el-input>
        </el-form-item>

        <el-form-item
          :label="$t('page_transfer.gasPrice')"
          class="czr-slider-gas"
        >
          ...
          <span class="wei-unit"> x 10<sup>-9</sup> CZR </span>
        </el-form-item>
        ...
        <el-form-item>
          <el-input
            type="textarea"
            :rows="4"
            :placeholder="$t('page_transfer.data_placeholder')"
            v-model="extraData"
          ></el-input>
        </el-form-item>
    ...
    <el-dialog
      :title="$t('page_transfer.contacts_dig.title')"
    ...
    >
      <span>
        <el-select
          ...
          :placeholder="$t('page_transfer.contacts_dig.select_placeholder')"
          style="width:100%;"
        >
          <el-option
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogSwitch.contacts = false">{{
          $t("cancel")
        }}</el-button>
        <el-button type="primary" @click="confrimContacts">{{
          $t("confirm")
        }}</el-button>
      </span>
    </el-dialog>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogSwitch.confrim = false">{{
          $t("cancel")
        }}</el-button>
        <el-button type="primary" @click="dialogSwitch.password = true">{{
          $t("confirm")
        }}</el-button>
      </div>
      ...
      <el-dialog
        ...
        :visible.sync="dialogSwitch.password"
        @open="openPwd"
        append-to-body
      >
        ...
        <div slot="footer" class="dialog-footer">
          <el-button @click="dialogSwitch.password = false">{{
            $t("cancel")
          }}</el-button>
          <el-button
            type="primary"
            :loading="sending"
            @keyup.enter.native="sendTransaction"
...
export default {
  ...
  watch: {
    ...
    txFee(newVal) {
      ...
      if (this.checkedAll) {
        const amountCanSend = new BigNumber(this.accountInfo.balance).minus(
          new BigNumber(newVal).times("1e18")
        );
  ...
  async created() {
    ...
    function tryTimes(task, times, res, rej) {
      let i = 0;
      function runTask() {
        task()
          .then(ret => {
  ...
  methods: {
    ...
    inputAmount(val, ref, dataPath) {
      ...
      const pos = $input.selectionStart;
      if (
        /(^[123456789]\d*(\.?\d{0,18})?$)|(^0((?=\.)\.?\d{0,18})?$)/.test(
          val
        ) ||
        val === ""
      ) {
          ...
          .then(res => {
            if (res.code !== 0) {
              switch (res.code) {
                ...
                case 7:
                  this.$message.error(
                    this.$t("rpcErrors.invalidGasPriceFormat")
                  );
                  break;
                case 8:
    ...
    getGasPrice() {
      return new Promise((resolve, reject) => {
        axios
          .get(
            `http://apis.canonchain.com/apis?apikey=BYBA6sS782wXtc4xnEUp34hBpCvztqRm69h4NFADu7gN&module=other&action=gas_price&t=${Date.now()}`
          )
          .then(res => {
            // console.log('axios.get',res)
            if (res.status !== 200) {
              this.$walletLogs.info(`获取gas Price失败`, res);
            ...
            this.gasPriceRange.low = new BigNumber(cheapest_gas_price)
              .div("1e9")
              .toString();
            this.gasPriceRange.medium = new BigNumber(median_gas_price)
              .div("1e9")
              .toString();
            this.gasPriceRange.high = new BigNumber(highest_gas_price)
              .div("1e9")
              .toString();
    ...
    changeSlider(val) {
      if (this.checkedAll) {
        const weiVal = this.accountInfo.balance;
        const targetVal = self.$czr.utils.fromKing(
          new BigNumber(weiVal)
            .minus(
              new BigNumber(this.gas).times(new BigNumber(val)).times("1e9")
            )
            .toString(),
          "czr"
        );
    ...
    async validateForm() {
      // const self = this;
      const reg = /^\d+(\.\d{1,18})?$/;
      const regObj = reg.exec(self.amount);
      // const czrAmount = parseFloat(self.$czr.utils.toKing(self.amount, "czr"));
      parseFloat(self.$czr.utils.toKing(self.amount, "czr"));

      if (!self.toAccount) {
        self.$message.error(self.$t("page_transfer.msg_info.address_null"));
        return;
      }

      // 发送金额 非数字，不可发送
      if (!regObj) {
        self.$message.error(self.$t("page_transfer.msg_info.amount_error"));
        return;
      }

      if (!reg.test(this.gas)) {
        self.$message.error(self.$t("page_transfer.msg_info.gas_error"));
        return;
      }

      if (!/^\d+(\.\d{1,9})?$/.test(this.gasPrice)) {
        self.$message.error(self.$t("page_transfer.msg_info.gasPrice_error"));
        return;
      }

      const gasValue = new BigNumber(this.gas).times(
        new BigNumber(this.gasPrice).times("1e9")
      );
      if (gasValue.lt(new BigNumber("1e7"))) {
        self.$message.error(self.$t("page_transfer.msg_info.txFeeTooLow"));
        return;
      }
      ...
      try {
        ...
        if (res.code !== 0) {
          ...
          this.$walletLogs.error(res.msg);
          // console.error(res.msg);
          return;
        }
        if (new BigNumber(this.gas).lt(new BigNumber(res.gas))) {
          this.$message.error("gas too low");
          this.$walletLogs.info("gas too low", this.gas, res.gas);
          // console.error(this.gas, res.gas);
          return;
        }
      } catch (e) {
        this.$walletLogs.info("$czr.request.estimateGas Error", e.stack);
        // console.error(e);
        return;
      }
      // }
      ...
      // TODO gas_price待确定
      const amountValue = self.$czr.utils.toKing(this.amount, "czr");
      if (
        new BigNumber(amountValue)
          .plus(new BigNumber(gasValue))
          .gt(this.accountInfo.balance)
      ) {
        self.$message.error(self.$t("page_transfer.msg_info.amount_exceeded"));
        return;
      }
      ...
      self.$czr.request
        ...
        .catch(err => {
          ...
          this.$walletLogs.error("Failed to validate account", err.stack);
        ...
        .then(data => {
          ...
          if (data === 1) {
            ...
            self.$message.error(self.$t("page_transfer.msg_info.address_err"));
          }
        });
    },
    openPwd() {
    ...
    async sendTransaction() {
      ...
      const amountValue = self.$czr.utils.toKing(this.amount, "czr");
      const gasValue = this.gas;
      const gasPrice = new BigNumber(this.gasPrice).times("1e9").toString();
       ...
       */
      const keystore = await self.$nedb.accounts_keystore.findOne({
      ...
      try {
        if (
          await self.$czr.accounts.validate_account(
            keystore,
            self.fromInfo.password
          )
        ) {
          privateKey = await self.$czr.accounts.decrypt(
            keystore,
            self.fromInfo.password
          );
        } else {
      ...
      try {
        ...
        self.$message.error(
          self.$t("page_transfer.msg_info.generate_offline_block_err")
        );
        self.isSubmit = false;
        this.sending = false;
        // console.log(e)
      ...
      try {
        ...
        self.$message.error(
          self.$t("page_transfer.msg_info.send_offline_block_err")
        );
        self.isSubmit = false;
        this.sending = false;
        // console.log(e)
      ...
      if (res.code === 0) {
        ...
        self.$message.success(self.$t("page_transfer.msg_info.send_success"));
        // Clear data
