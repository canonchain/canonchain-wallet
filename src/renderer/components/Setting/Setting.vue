    ...
    <div class="setting-content">
      ...
      <el-form label-position="right" label-width="340px">
        ...
        <el-form-item :label="$t('page_setting.data_path')">
          <p>{{ data_path }}</p>
          <el-button @click="changeDataPath">{{
            $t("page_setting.change")
          }}</el-button>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async init() {
      const val = await this.$nedb.setting_language_active.findOne({
        name: "active"
      });
      const pat = await this.$nedb.setting_node_path.findOne({
        name: "node_path"
      });
      this.value = val.value;
      this.data_path = pat.path;

      const langsArray = await this.$nedb.setting_language.find();
    ...
    changeDataPath() {
      this.$alert(
        this.$t("page_setting.changeDirNote"),
        this.$t("page_setting.note")
      ).then(async () => {
        ...
        // @jiandong  异步更新节点保存路径
        await this.$nedb.setting_node_path.update(
          { name: "node_path" },
          { $set: { path: dir } }
        );
        remote.app.relaunch();
    ...
    async selectVal(val) {
      ...
      // @jiandong  异步改变语言
      await this.$nedb.setting_language_active.update(
        { name: "active" },
        { $set: { value: val } }
      );
